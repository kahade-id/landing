    ...
    <div className={`faq-item faq-fade-up ${inView ? "fv" : ""} ${delayClass} px-0 border-b border-black/10 ${index === 0 ? "border-t" : ""}`}>
      <button onClick={() => setOpen(!open)} id={buttonId} className="w-full flex items-center justify-between py-5 text-left gap-4 group" aria-expanded={open} aria-controls={panelId}>
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <span className={`text-[14.5px] sm:text-[15px] font-semibold leading-snug transition-colors ${open ? "text-black" : "text-black/75 group-hover:text-black"}`}>{faq.q}</span>
        </div>
      ...
      <div id={panelId} role="region" aria-labelledby={buttonId} className={`faq-answer-wrap ${open ? "open" : ""}`}>
        <div className="faq-answer-inner">
          <div className="faq-answer-content pb-5 pl-8">
            <p className="text-[13.5px] sm:text-[14px] text-black/55 leading-relaxed mb-3">{faq.a}</p>
            <div className="flex flex-wrap gap-1.5">
              {faq.tags.map((tag, ti) => (
                <span key={ti} className="px-2 py-0.5 rounded-md bg-black/[.04] text-[10.5px] font-semibold text-black/40 border border-black/6">{tag}</span>
              ))}
            </div>
          </div>
        </div>
...
export default function FAQSection() {
  ...
  return (
    <section id="faq" ref={sectionRef} className="faq-root relative overflow-hidden py-20 sm:py-28 lg:py-36" style={{ background: "#FFFFFF" }}>
      <AbstractBg />
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className={`${cls("faq-fade-up", inView, "faq-d1")} text-[2rem] sm:text-[2.5rem] lg:text-[2.8rem] font-extrabold text-black leading-[1.1] tracking-tight mb-5`} style={{ fontFamily: "var(--font-display)" }}>
