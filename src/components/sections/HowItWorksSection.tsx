...
const icons: Record<string, (w: boolean) => React.ReactNode> = {
  ...
  unlock: (w) => <Icon white={w} d={<><path d="M11 3.5a4 4 0 014 4V9H5v10a2 2 0 002 2h8a2 2 0 002-2V9h-2V7.5a4 4 0 00-4-4z" opacity=".35" stroke="none" fill={w?"rgba(255,255,255,.2)":"rgba(0,0,0,0.07)"}/><rect x="3" y="9" width="16" height="10" rx="2"/><path d="M11 13v3M9.5 16.5l1.5-1 1.5 1"/><path d="M7.5 9V7.5a3.5 3.5 0 015.9-2.5"/></>} /> };

const ArrowRight = ({ white = false }) => (
  <svg width="13" height="13" viewBox="0 0 13 13" fill="none"
    stroke={white ? "rgba(255,255,255,.6)" : "rgba(0,0,0,0.4)"}
...
const StepCard = ({ step, inView, delayIdx }: { step: any; inView: boolean; delayIdx: number }) => {
  ...
  const textSecond   = isF ? "rgba(255,255,255,.45)"  : "rgba(0,0,0,0.45)";
  const textBody     = isF ? "rgba(255,255,255,.68)"  : "rgba(0,0,0,0.62)";
  const borderColor  = isF ? "rgba(255,255,255,.1)"   : "rgba(0,0,0,0.09)";
  const iconBg       = isF ? "rgba(255,255,255,.1)"   : "rgba(0,0,0,0.06)";
  const numBg        = isF ? "rgba(255,255,255,.1)"   : "rgba(0,0,0,0.06)";
  const tagBg        = isF ? "rgba(255,255,255,.1)"   : "rgba(0,0,0,0.06)";
  ...
  return (
    ...
    >
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:22, gap:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div className="hw-num-badge" style={{ background:numBg }}>
            <span style={{ fontWeight:800, color:isF?"rgba(255,255,255,.6)":"rgba(0,0,0,0.42)", letterSpacing:".02em" }}>
              {step.num}
            </span>
          </div>
          {/* Icon box */}
        ...
        {/* Tag */}
        <div style={{
          padding:"5px 11px", background:tagBg, border:`1px solid ${tagBorder}`,
          fontWeight:700, color:isF?"rgba(255,255,255,.55)":"rgba(0,0,0,0.45)",
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:20 }}>
        {step.details.map((d: string, i: number) => (
          <div key={i} style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <span style={{ fontWeight:500, color:textSecond }}>{d}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingTop:14, borderTop:`1px solid ${borderColor}` }}>
        <div style={{ display:"flex", alignItems:"center", gap:7, padding:"5px 11px", background:timeBg }}>
          <svg width="11" height="11" viewBox="0 0 11 11" fill="none"
            stroke={isF?"rgba(255,255,255,.4)":"rgba(0,0,0,0.1)"} strokeWidth="1.4" strokeLinecap="round">
            <circle cx="5.5" cy="5.5" r="4.5"/>
            <path d="M5.5 3v2.8l1.5 1.2"/>
          </svg>
          <span style={{ fontWeight:600, color:isF?"rgba(255,255,255,.4)":"rgba(0,0,0,0.38)" }}>
            {step.time}
          </span>
        </div>
  ...
  const content = (
    <div className={`hw-${isEven?"fl":"fr"} hw-d${2+idx} ${inView?"hv":""}`}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
        <div style={{
          ...
          <span style={{ fontWeight:800,
            color: isF ? "rgba(255,255,255,.55)" : "rgba(0,0,0,0.45)",
            letterSpacing:".03em"
          }}>{step.num}</span>
        </div>
        <div style={{
          ...
          fontWeight:700, color:"rgba(0,0,0,0.45)",
        ...
        <div style={{ display:"flex", alignItems:"center", gap:6 }}>
          ...
          <span style={{ fontWeight:600, color:"rgba(0,0,0,0.45)" }}>{step.time}</span>
        </div>
      </div>
      ...
      <h3 style={{
        ...
        fontWeight:800, letterSpacing:"-.04em", lineHeight:1.08 }}>{step.label}</h3>

      {/* Body */}
      <p style={{
        fontSize:"clamp(14px,1.3vw,15.5px)",
        lineHeight:1.78, color:"rgba(0,0,0,0.45)",
        maxWidth:420
      }}>{step.desc}</p>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {step.details.map((d: string, i: number) => (
          <div key={i} style={{ display:"flex", alignItems:"center", gap:10 }}>
            <CheckSmall/>
            <span style={{ fontWeight:500, color:"rgba(0,0,0,0.45)" }}>{d}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  const visual = (
    <div className={`hw-${isEven?"fr":"fl"} hw-d${3+idx} ${inView?"hv":""}`}
      ...
      <div className="hw-float" style={{
        width:100, height:100, borderRadius: "var(--radius-card)",
      ...
      <div className="hw-float" style={{
        ...
        <div style={{ fontWeight:700, color:"rgba(0,0,0,0.45)", textTransform:"uppercase", letterSpacing:".08em", marginBottom:2 }}>
          Status
        </div>
      ...
      <div className="hw-float" style={{
        ...
        background:"#000", borderRadius: "var(--radius-md)", padding:"8px 14px",
        ...
        <span style={{ fontWeight:600, color:"rgba(255,255,255,.8)", whiteSpace:"nowrap" }}>
          {step.time}
        </span>
      </div>
    </div>
      ...
      >
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div style={{ textAlign:"center", marginBottom:56 }}>
            ...
            <div className={c("hw-fu hw-d0")} style={{ display:"flex", justifyContent:"center", marginBottom:20 }}>
              <div style={{
                ...
                <span style={{ fontWeight:600, color:"rgba(0,0,0,0.45)", letterSpacing:".04em", textTransform:"uppercase" }}>
                  5 Langkah Sederhana
                </span>
              </div>
            </div>
            ...
            <div className={c("hw-fu hw-d1")}>
              <h2 style={{
                ...
                Dari Kesepakatan
                <br/>
                <span style={{ color:"rgba(0,0,0,0.45)" }}>Hingga Dana Cair.</span>
              </h2>
            </div>
                ...
                {steps.map((s,i)=>(
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      <span style={{
                        fontWeight:800,
                        color: s.featured ? "rgba(255,255,255,.6)" : "rgba(0,0,0,0.42)",
                        letterSpacing:".02em"
                      }}>{s.num}</span>
                    </div>
          ...
          <div className={c("hw-fu hw-d8")} style={{ marginTop:64 }}>
            <div style={{
              background:"#000", borderRadius: "var(--radius-card)", overflow:"hidden",
              ...
              <div className="relative z-10">
                <p style={{ fontSize:"clamp(17px,2.5vw,22px)", fontWeight:800, letterSpacing:"-.04em", margin:"0 0 6px 0" }}>
                  Siap memulai transaksi pertama Anda?
                </p>
                <p style={{ color:"rgba(255,255,255,.42)" }}>
