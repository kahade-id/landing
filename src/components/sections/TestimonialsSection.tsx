  ...
  return (
    <div className={`tm-card${isF ? " featured" : ""}`} style={compact ? { minWidth: 300, maxWidth: 320, flexShrink: 0 } : {}}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: nameColor, letterSpacing: "-0.02em" }}>
                {t.name}
              </span>
              {isF ? <VerifiedIconWhite /> : <VerifiedIcon />}
            </div>
            <p style={{ fontSize: "var(--text-xs)", color: metaColor, fontWeight: 500 }}>
              {t.role} · {t.location}
            </p>
          </div>
        </div>
      ...
      <p style={{
        ...
        lineHeight: 1.72, color: textColor, fontStyle: "italic", fontWeight: 400,
      ...
      <div style={{
        ...
        <div>
          <p style={{ fontSize: "var(--text-2xs)", color: metaColor, margin: "0 0 2px 0", fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.08em" }}>
            {t.txLabel}
          </p>
          <p style={{ fontWeight: 800, color: nameColor, letterSpacing: "-0.03em" }}>
            {t.amount}
          </p>
        </div>
        <div style={{
          display: "flex", alignItems: "center", gap: 5,
          padding: "6px 10px", background: isF ? "var(--color-white-15)" : "var(--color-ink-07)" }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: isF ? "var(--color-white-80)" : "var(--color-ink)", display: "block" }} />
          <span style={{ fontSize: "var(--text-2xs)", fontWeight: 600, color: isF ? "var(--color-white-80)" : "var(--color-ink-60)" }}>
            Terverifikasi
          </span>
        </div>
      </div>
...
const MarqueeCard = ({ t }: { t: (typeof testimonials)[number] }) => (
  <div className="tm-marquee-card">
    <div style={{ position: "absolute", top: 14, right: 18, fontSize: 44, lineHeight: 0.6, color: "var(--color-ink-07)", fontWeight: 800, pointerEvents: "none" }}>"</div>
    <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
      <div style={{ width: 34, height: 34, borderRadius: "50%", background: t.avatarBg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        <span style={{ fontSize: "var(--text-2xs)", fontWeight: 700, color: "var(--color-surface)" }}>{t.avatar}</span>
      </div>
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          <span style={{ fontSize: "var(--text-sm)", fontWeight: 700, color: "var(--color-ink)", letterSpacing: "-0.02em" }}>{t.name}</span>
          <VerifiedIcon />
        </div>
        <p style={{ fontSize: "var(--text-2xs)", color: "var(--color-ink-45)" }}>{t.role}</p>
      </div>
      <div style={{ marginLeft: "auto" }}>
        <Stars count={5} />
      </div>
    </div>
    <p style={{ fontSize: "var(--text-sm)", lineHeight: 1.68, color: "var(--color-ink-60)", fontStyle: "italic" }}>
